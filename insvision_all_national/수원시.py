...
'''앞 8개 그래프 : 매매가격지수 그래프'''
filtered_data1 = seoul_weekly_tx[seoul_weekly_tx['CLS_NM'].isin(['전국', '수원시'])]
filtered_data2 = seoul_weekly_tx[seoul_weekly_tx['CLS_NM'].isin(['6대광역시', '수원시'])]
'''뒤 8개 그래프 : 전세가격지수 그래프'''
filtered_data5 = seoul_weekly_res[seoul_weekly_res['CLS_NM'].isin(['전국', '수원시'])]
filtered_data6 = seoul_weekly_res[seoul_weekly_res['CLS_NM'].isin(['6대광역시', '수원시'])]
...
fig2.update_layout(
    title={
        'text': '매매가격지수 최근 26주 수원시 6대광역시 비교',
        'y': 0.95,
        'x': 0.5,
        'xanchor': 'center',
        'yanchor': 'top',
...
fig3.update_layout(
    title={
        'text': '매매가격지수 등락률 최근 26주 수원시 전국 비교',
        'y': 0.95,
        'x': 0.5,
        'xanchor': 'center',
        'yanchor': 'top',
...
'''4번그래프 : 아파트 매매가격지수 등락률 수원시 6대광역시 비교'''
# 전월 대비 등락률 계산
...
fig4.update_layout(
    title={
        'text': '매매가격지수 등락률 최근 26주 수원시 6대광역시 비교',
        'y': 0.95,
        'x': 0.5,
        'xanchor': 'center',
        'yanchor': 'top',
...
fig10.update_layout(
    title={
        'text': '전세가격지수 최근 26주 수원시 6대광역시 비교',
        'y': 0.95,
        'x': 0.5,
        'xanchor': 'center',
        'yanchor': 'top',
...
fig11.update_layout(
    title={
        'text': '전세가격지수 등락률 최근 26주 수원시 전국 비교',
        'y': 0.95,
        'x': 0.5,
        'xanchor': 'center',
        'yanchor': 'top',
...
'''12번그래프 : 전세가격지수 등락률 수원시 6대광역시 비교'''
# 전월 대비 등락률 계산
...
fig12.update_layout(
    title={
        'text': '전세가격지수 등락률 최근 26주 수원시 6대광역시 비교',
        'y': 0.95,
        'x': 0.5,
        'xanchor': 'center',
        'yanchor': 'top',
...
# 수원시 구별 매매가격지수 바 차트 생성
fig17 = px.bar(latest_data,
               x='CLS_NM',  # 구 이름
               y='DTA_VAL',  # 매매가격지수
               color='CLS_NM',  # 구에 따른 색상 구분
               title=f"수원시 구별 최근 주차 매매가격지수 ({latest_week})",  # 그래프 제목
               )
...
fig17.update_layout(
    ...
    annotations=[
        dict(
            ...
            yanchor='bottom',  # 아래쪽 끝에 맞춤
            showarrow=False,  # 화살표 없이 텍스트만 표시
            font=dict(size=15, color="black")  # 작은 회색 글씨로 표시
        )
    ]
)
...
for i in range(len(latest_data)):
    fig17.add_annotation(
        ...
        showarrow=False,
        font=dict(size=15, color='black'),  # 폰트 크기와 색상
        yshift=10  # 바 위에 위치하도록 y축을 약간 위로 이동
    )

'''18번그래프 : 수원시 전체 구 최근 주차 전세가격지수'''
# 최신 주차 데이터 필터링
...
fig18 = px.bar(latest_data2,
               ...
               y='DTA_VAL',  # 매매가격지수
               color='CLS_NM',  # 구에 따른 색상 구분
               title=f"수원시 구별 최근 주차 전세가격지수 ({latest_week2})",  # 그래프 제목
               )
